...
function getWallet(chain) {
    ...
    return new Wallet(deployerKey, provider);
}
...
async function upgradeTokenLinker(chains) {
    for (const chain of chains) {
        ...
        await upgradeUpgradable(chain.tokenLinker, wallet, implContractJson, args, '0x');
    }
}
...
async function deployConstAddressDeployer(chains) {
    for (const chain of chains) {
        ...
        const wallet = getWallet(chain);
        const deployerWallet = new Wallet(deployerPrivateKey, wallet.provider);
...
async function sendToken(fromChain, toChain, to, amount, gasPrice) {
    ...
    if (fromChain.name === 'Subnet') {
        ...
        await (
            await tokenLinker.sendToken(toChain.name, to, amount, { value: BigInt(gasPrice) * BigInt(gasLimit) + BigInt(amount) })
        ).wait();
    } else {
