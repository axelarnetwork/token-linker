...
(async () => {
    ...
    for(const contractJson of [LockUnlock, MintBurn, MintBurnExternal, Native]) {
        impl.push(
            await deployContract(wallet, contractJson, [chain.gateway.address, chain.gasReceiver.address]),
        )
    }
    const bytecode = Proxy.bytecode;
    const codehash = keccak256(bytecode);
    const factory = await deployContract(wallet, TokenLinkerFactory, [...impl.map((i => i.address)), codehash, chain.gateway.address, chain.gasReceiver.address]);
    
    const tx = await (await factory.deploy(3, keccak256(toUtf8Bytes('asdasd')), '0x')).wait();
    const id = tx.events[0];
})()